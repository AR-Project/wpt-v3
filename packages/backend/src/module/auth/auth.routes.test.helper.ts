...
export async function signUpHelper(user: SignUpPayload, appInstance: HonoApp) {
...
export async function signInHelper(
	...
	return {
		id: json.user.id,
		cookie,
		defaultCategoryId: json.user.defaultCategoryId!,
	};
}

/** Test helper for sign-up, then sign-in a user. Return cookie. Remember to clean up default category */
export async function signUpSignInHelper(
	user: SignUpPayload,
	appInstance: HonoApp,
): Promise<{ id: string; cookie: string; defaultCategoryId: string }> {
	await signUpHelper(user, appInstance);
	const { id, cookie, defaultCategoryId } = await signInHelper(
