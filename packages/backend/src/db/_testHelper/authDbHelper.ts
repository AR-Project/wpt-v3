...
export const authTableHelper = {
	...
	clean: async (option?: CleanOption) => {
		await db
			.delete(session)
			.where(option?.userId ? eq(session.userId, option.userId) : undefined);
		await db
			.delete(account)
			.where(option?.userId ? eq(account.userId, option.userId) : undefined);
		await db
			.delete(user)
			.where(option?.userId ? eq(user.id, option.userId) : undefined);
	},
