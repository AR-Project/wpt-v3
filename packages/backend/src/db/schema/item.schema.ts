...
export const item = sqliteTable(
  ...
  (t) => ({
    ...
    userIdParent: t.text("user_id_parent")
      .notNull()
      .references(() => user.id, {
        onDelete: "cascade",
      }),
    userIdCreator: t.text("user_id_creator")
      .notNull()
      .references(() => user.id, {
        onDelete: "cascade",
      }),
    ...
    createdAt: t.integer("created_at", { mode: "timestamp_ms" })
      .$defaultFn(() => new Date())
      .notNull(),
    modifiedAt: t.integer("last_modified_at", { mode: "timestamp_ms" })
      .$onUpdateFn(() => new Date())
      .notNull(),
  })
  ,
  (table) => [
    ...
    uniqueIndex("item_owner_idx").on(table.userIdParent),
    uniqueIndex("item_creator_idx").on(table.userIdCreator),
  ]
);
